...
class RequestStatusLog(Base, TimestampMixin):
    ...
    request_id: Mapped[int] = mapped_column(ForeignKey("delivery_requests.id", ondelete="CASCADE"), nullable=False)
    old_status: Mapped[RequestStatus | None] = mapped_column(Enum(RequestStatus), nullable=True)
    new_status: Mapped[RequestStatus] = mapped_column(Enum(RequestStatus), nullable=False)
    changed_by_user_id: Mapped[int | None] = mapped_column(ForeignKey("users.id"), nullable=True)
    note: Mapped[str | None] = mapped_column(String(255), nullable=True)

    request: Mapped["DeliveryRequest"] = relationship(back_populates="status_logs")
